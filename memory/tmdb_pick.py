...
# Liked
rows = conn.execute("""
    SELECT title, my_rating, genres, countries, comment
    FROM douban_watch_history
    WHERE status='collect' AND my_rating>=4
    ORDER BY my_rating DESC LIMIT 20
""").fetchall()
for r in rows:
...
rows2 = conn.execute("""
    ...
    FROM douban_watch_history
    WHERE status='collect' AND my_rating<=2 AND my_rating>0
""").fetchall()
for r in rows2:
