...
# Already recommended IDs to exclude
...
# Search Korean thriller/crime/fantasy TV
print("=== Korean thriller/crime TV ===")
r = requests.get("https://api.themoviedb.org/3/discover/tv", params={
    ...
    "vote_count.gte": 200,
    "with_genres": "18,80,10765",  # Drama, Crime, Sci-Fi
    "with_origins": "KR",
    "page": 1
...
for item in data.get("results", [])[:10]:
    if item["id"] not in ALREADY:
        print(item["id"], item["name"], item["vote_average"], item["vote_count"])

# Search US sci-fi/thriller
print("\n=== US Sci-fi/Thriller Movies ===")
...
for item in data.get("results", [])[:10]:
    if item["id"] not in ALREADY:
        print(item["id"], item["title"], item["vote_average"], item["vote_count"])
