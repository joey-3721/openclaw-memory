...
# Check known Korean show IDs
ids = {
    "95403": "Sweet Home",
    "95479": "All of Us Are Dead", 
    "69050": "Kingdom",
    "2317": "My Name",
    "2316": "The Office",
    "141809": "Moving",
...
for tid, label in ids.items():
    ...
    if d.get("success") == False:
        print(f"NOT FOUND: {tid} ({label})")
        continue
    vote_avg = d.get("vote_average", 0)
    vote_cnt = d.get("vote_count", 0)
    orig_lang = d.get("original_language")
    orig_c = d.get("origin_country", [])
    flag = "**" if vote_avg >= 7.5 and vote_cnt >= 200 else "  "
    print(f"{flag} {tid} | {d.get('name')} | {d.get('original_name')} | {orig_lang} | {orig_c} | rating:{vote_avg} votes:{vote_cnt}")
