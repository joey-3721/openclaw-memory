...
- 心跳新增推荐维护规则：每次心跳都检查 `recommended` 列表可见数量（排除 `dislike`）。如果少于 20 个，就基于佳奕最近看过、最近不喜欢、长期画像，按 `tmdb-recommendation-flow` skill 精挑细选**只新增 1 个**不重复推荐；如果已达到 20 个或更多，就先不新增。
- 新增推荐必须认真写 `recommendation_note`：不能只改写简介，默认要结合全网搜索/剧情梳理/口味匹配来写，重点说明为什么现在值得点开。
- **强制写入路径规则**：任何推荐写入脚本（如 `cron_add_recommendation.py`）必须只能写入 `MEDIA_HUB_DB`（生产卷路径 `/app/data/douban_media.db`），禁止写入 workspace 开发副本路径。脚本已内嵌安全检查，非生产路径会直接拒绝执行。
